...
import dvp.Template;
import dvp.Test;
import dvp.TestResult;
import dvp.TestWrapper;
...
public class DatabaseTransactions {
    ...
    /**
     * Insert a component pdf along with uploaded subparts. Also inserts parent-child connections to catia.BOM table
     *
     ...
     *
     * @param editedTestWrapper the dvp.TestWrapper instance that has updated information
     * @param parentTest        the dvp.Test the dvp.TestWrapper belongs to
     * @param testResult        the dvp.TestResult that was updated in the dvp.TestWrapper
     */
    public void editTestWrapper(TestWrapper editedTestWrapper, Test parentTest, TestResult testResult) throws SQLException {
        ...
        try {
            ...
            DbContext.getConnection().setAutoCommit(true);
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setHeaderText("dvp.Test result edited succesfuly!");
            alert.showAndWait();
        }
    }
    ...
    public void insertTemplate(Template template) throws SQLException {
        ...
        try {
            ...
            DbContext.getConnection().rollback();
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("dvp.Template upload not succesful!");
            alert.showAndWait();
            throw new RuntimeException(e);
        } finally {
            DbContext.getConnection().setAutoCommit(true);
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setHeaderText("dvp.Template uploaded succesfuly!");
            alert.showAndWait();
        }
    }
    ...
    public void deleteTemplate(Template template) throws SQLException {
        ...
        try {
            ...
            DbContext.getConnection().setAutoCommit(true);
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setHeaderText("dvp.Template deleted succesfuly!");
            alert.showAndWait();
        }
    }
    ...
    /**
     * Deletes a catia.CatiaSheet from the database
     *
    ...
    public void deleteCatiaSheet(String partID) throws SQLException {
        ...
        try {
            ...
            DbContext.getConnection().rollback();
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Failed to delete catia.CatiaSheet");
            alert.showAndWait();
            throw new RuntimeException(e);
        } finally {
            dc.insert();
            DbContext.getConnection().setAutoCommit(true);
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setHeaderText("catia.CatiaSheet deleted succesfuly!");
            alert.showAndWait();
        }
    }
    ...
    public static void insertUser(String mail, String password, int admin) throws SQLException {
        ...
        try {
            ...
            DbContext.getConnection().setAutoCommit(true);
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setHeaderText("backend.User added succesfully!");
            alert.showAndWait();
        }
