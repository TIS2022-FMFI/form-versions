...
public class AdminController implements Initializable {
    ...
    public void addNewUser(ActionEvent actionEvent) throws SQLException {
        if (newMailTextField == null || Objects.equals(newMailTextField.getText(), "")){
            ...
            alert.setHeaderText("Please fill in Email of new backend.User !");
            alert.showAndWait();
        }

        else if (newPasswordField == null || Objects.equals(newPasswordField.getText(), "")){
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Please fill in password of new backend.User ! ");
            alert.showAndWait();
        }
        else {
            ...
            try {
                if (UserFinder.checkIfCanAddNewUser(newMailTextField.getText())) {
                    ...
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setHeaderText("backend.User with the same email adress is already in the System ! Please try a different one.");
                    alert.showAndWait();
                }
            } catch (Exception e) {
                e.printStackTrace();
